...
#define FUNC [](const string_vec& vec, const std::string& cmd) -> Command*
    ...
    static std::map<std::string, handler_type, CaseInsensitiveLess> handlers {
        {"list", FUNC { return new ListCommand; }},										//输入list就是返回new的listcommand
...
void CloneCommand::execute(void){
	std::cout<<"class:CloneCommand method:excute\n";
	scene->clone();
}
